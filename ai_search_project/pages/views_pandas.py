...
#read_json_file("data/product.json")
...
def search(request):
    """
    Search the product DataFrame. Every column is searched
    case-insensitively; a row matches if ANY column contains the query.
    ...
    print("LEN PRODUCTS_DF = ", PRODUCTS_DF)
    if PRODUCTS_DF.empty:
        error = f"No product data loaded. Check that {DATA_FILE} exists and is valid JSON."
        return render(request, 'search_results.html', {
            'results': [], 'query': query, 'error': error, 'num_found': 0,
        })

    try:
        # Build a mask: for each word in the query, the row matches if
        # ANY column contains the word (case-insensitive).
        words = query.lower().split()
        mask = pd.Series(True, index=PRODUCTS_DF.index)
        for word in words:
            word_mask = PRODUCTS_DF.apply(
        ...
        # Limit to first 25 rows for display
        for _, row in matched.head(25).iterrows():
            row_dict = row.to_dict()

            # Best-effort guesses at common field names – adjust to your schema
            title = (row_dict.get('product_id')
                     or row_dict.get('product_name')
                     or row_dict.get('product_class')
                     or 'Untitled')

            snippet_source = (row_dict.get('category_hierarchy')
                              or row_dict.get('product_class')
                              or row_dict.get('product_description')
                              or '')
            ...
            results.append({
                ...
                'row':     row_dict,   # full record available if you want it
            })
